...
//==================================================//
const thumb = fs.readFileSync ('./ShunAi/media/image/thumb.jpg')
const vnnye = JSON.parse(fs.readFileSync('./ShunAi/media/vn.json'))
const docunye = JSON.parse(fs.readFileSync('./ShunAi/media/docs.json'))
const zipnye = JSON.parse(fs.readFileSync('./ShunAi/media/zip.json'))
const apknye = JSON.parse(fs.readFileSync('./ShunAi/media/apk.json'))
const vidnye = JSON.parse(fs.readFileSync('./ShunAi/media/vid.json'))
const pengguna = JSON.parse(fs.readFileSync('./ShunAi/database/database.json'))
let premium = JSON.parse(fs.readFileSync('./ShunAi/database/premium.json'));
const banned = JSON.parse(fs.readFileSync('./ShunAi/database/banned.json'))
//==================================================//
module.exports = hardi = async (hardi, m, chatUpdate, store) => {
 try {
...
const botNumber = await hardi.decodeJid(hardi.user.id)
...
const groupMetadata = m.isGroup ? await hardi.groupMetadata(from).catch(e => {}) : ''
...
const bii = JSON.parse(fs.readFileSync('./ShunAi/database/database.json').toString())
...
// FUNCTION
try {
pplu = await hardi.profilePictureUrl(anu.id, 'image')
} catch {
...
const reply = async(teks) => {hardi.sendMessage(from, {text: teks, mentions: await ments(teks)},{quoted:fcall})}

hardi.autoshalat = hardi.autoshalat ? hardi.autoshalat : {}
	let who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? hardi.user.id : m.sender
	let id = m.chat 
    if(id in hardi.autoshalat) {
    return false
    }
    let jadwalSholat = {
    shubuh: '04:29',
    ...
    reply(caption),
    setTimeout(async () => {
    delete hardi.autoshalat[m.chat]
    }, 57000)
    ]
    }
    }
...
.on('finish', async () => {
await hardi.sendMessage(from, { video: fs.readFileSync(mp4File), gifPlayback: false }, { quoted: fcall })
...
.on('finish', async () => {
await hardi.sendMessage(from, { audio: fs.readFileSync(mp3File), mimetype: 'audio/mp4' }, { quoted: fcall })
...
if (m.message) {
hardi.sendPresenceUpdate('available', m.chat)
console.log(
...
let gclink = (`https://chat.whatsapp.com/`+await hardi.groupInviteCode(m.chat))
let isLinkThisGc = new RegExp(gclink, 'i')
let isgclink = isLinkThisGc.test(m.text)
if (isgclink) return hardi.sendMessage(m.chat, {text: `\`\`\`「 Group Link Terdeteksi 」\`\`\`\n\nAnda tidak akan ditendang oleh bot karena yang Anda kirim adalah link ke grup ini`})
if (isAdmins) return hardi.sendMessage(m.chat, {text: `\`\`\`「 Group Link Terdeteksi 」\`\`\`\n\nAdmin sudah mengirimkan link, admin bebas memposting link apapun`})
if (isCreator) return hardi.sendMessage(m.chat, {text: `\`\`\`「 Group Link Terdeteksi 」\`\`\`\n\Owner telah mengirim link, owner bebas memposting link apa pun`})
await hardi.sendMessage(m.chat,
{
delete: {
remoteJid: m.chat,
fromMe: false,
...
hardi.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
hardi.sendMessage(from, {text:`\`\`\`「 Group Link Terdeteksi 」\`\`\`\n\n@${m.sender.split("@")[0]} Jangan kirim group link di group ini`, contextInfo:{mentionedJid:[sender]}}, {quoted:m})
}
}
//=================================================//
...
userJid: hardi.user.id,
quoted : m.quoted && m.quoted.fakeObj
})
messages.key.fromMe = areJidsSameUser(m.sender, hardi.user.id)
...
hardi.ev.emit('messages.upsert', msg)
}
//=================================================//
if (budy.startsWith('!')) {
try {
...
//==================================================//
try {
ppuser = await hardi.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
let setting = global.db.data.settings[botNumber]
		...
		await hardi.updateProfileStatus(`I am ${botname} | Aktif Selama ${uptime} ⏳ | Mode : ${hardi.public ? 'Public-Mode' : 'Self-Mode'}`).catch(_ => _)
        }


if (autoread) {
hardi.readMessages([m.key])
        }
...
let { key } = await hardi.sendReact(m.chat, `${pickMoji(['😨','😅','😂','😳','😎','🥵','😱','🐦','🙄','🐤','🗿','💬','🤨','🥴','😐','👆','😔','👀','👎','🥶','💯','🔥','👍','❓️','⏳️','💥','🤙'])}`, m.key)//Pengalih isu

for (let i = 0; i < Hardi.length; i++) {
await sleep(65)
await hardi.sendReact(m.chat, Hardi[i], m.key)
//PESAN LEPAS
}
}
...
function isShunBottz(jid) {
    ...
    return jid === '6281226680862@s.whatsapp.net';
}
...
let msg = generateWAMessageFromContent(m.chat, {
 ...
 businessMessageForwardInfo: { businessOwnerJid: hardi.decodeJid(hardi.user.id) },
 }, 
 ...
 subtitle: "dcdXdino",
 hasMediaAttachment: true,...(await prepareWAMessageMedia({ images: { url: "https://telegra.ph/file/592ec99e191cb993f993a.png" } }, { upload: hardi.waUploadToServer }))
 }),
...
await hardi.relayMessage(msg.key.remoteJid, msg.message, {
...
break
 ...
 await hardi.sendMessage(m.chat, {text: 'Tunggu Ya'})     
let media = await hardi.downloadAndSaveMediaMessage(quoted)
...
hardi.sendMessage(from, { image: buffer }, {quoted:m})
...
await hardi.sendMessage(m.chat, {image: {url: data.url.result}, caption: `Here u go!`}, {quoted: m})
}
break
...
wek = `${gris}「 I N F O  B O T 」${gris}
   ...
   𖦹 ᴍᴏᴅᴇ ʙᴏᴛ : *${hardi.public ? `ᴘᴜʙʟɪᴄ` : `sᴇʟғ`}*
...
let msg = generateWAMessageFromContent(m.chat, {
 ...
 title: ``,
 subtitle: "",
 hasMediaAttachment: true,...(await prepareWAMessageMedia({ image: { url: "https://telegra.ph/file/592ec99e191cb993f993a.png" } }, { upload: hardi.waUploadToServer }))
 }),
 nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
 buttons: [ 
 {
...
if (!text) await hardi.relayMessage(msg.key.remoteJid, msg.message, {
 messageId: msg.key.id
})
if (args[0] === "all") {
...
hardi.sendMessage(m.chat, {document: fs.readFileSync("./package.json"),
            fileName: `Powered By KaaKangSatir`,
...
thumbnailUrl: 'https://telegra.ph/file/d2e2ede8674b8adf67cd1.jpg',
sourceUrl: '', 
mediaType: 1,
...
case 'text2img': {
  ...
  if (args[0] && versionRegex.test(args[0])) {
    ...
    hardi.sendMessage(from, { image: { url: imgg }, caption: cap }, { quoted: m });
    return;
  }
...
hardi.sendMessage(m.chat, { text: gpt4.result }, { quoted: Shun })
}
break
...
hardi.sendMessage(m.chat, { text: blackbox.result }, { quoted: Shun })
}
break
...
hardi.sendMessage(m.chat, { text: gemini.result }, { quoted: Shun })
}
break
...
let cap = `${mess.success}`
hardi.sendMessage(from, {image: { url: json.url },caption: cap},{ quoted:m })
} else if (args[0] === "nsfw") {
...
.then(({data}) => {
hardi.sendImageAsSticker(m.chat, data.url, m, { packname: global.packname, author: global.author })
})
break
//—————「 ANIME MENU 」—————//
...
await hardi.sendMessage(m.chat, { image: { url: k }}, { quoted: fcall });
})
} else {
hardi.sendMessage(m.chat, { video: { url: tiktuk.result.video_hd }, caption: `${gris}[ T I K T O K - D O W N L O A D E R ]${gris}\n\n${hiasan}*Author* : ${tiktuk.result.author.nickname}\n${hiasan}*Desc* : ${tiktuk.result.desc}\n${hiasan}*Statistic* : ${tiktuk.result.statistic}\n${hiasan}*Fetching* : ${((new Date - old) * 1)} ms\n\n` }, { quoted: fcall })
await sleep(500)
hardi.sendMessage(m.chat, { audio: { url: tiktuk.result.music }, fileName: `tiktok.mp3`, mimetype: 'audio/mp4' })
}
}
break
*/
...
hardi.sendMessage(m.chat, { video: { url: iganu[0].url }, caption: mess.success })
}
break
//—————「 DOWNLOAD MENU 」—————//
...
let cap = `${mess.success}`
hardi.sendMessage(from, {image: { url: carbon.result },caption: cap},{ quoted:m })
}
break
...
case 'script': {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        ...
                        header: proto.Message.InteractiveMessage.Header.create({
                            ...
                            ...(await prepareWAMessageMedia(
                                { image: { url: `https://telegra.ph/file/d2e2ede8674b8adf67cd1.jpg` } },
                                { upload: hardi.waUploadToServer },
                            )),
                        }),
                        nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
                            buttons: [{
   ...
   if (!m.isGroup) return hardi.relayMessage(msg.key.remoteJid, msg.message, {
        messageId: msg.key.id,
    });
    
   if (m.isGroup) return hardi.relayMessage(m.chat, {
...
`;
    ...
    return hardi.relayMessage(msg.key.remoteJid, msg.message, {
        messageId: msg.key.id,
    });
}
break*/
...
case 'owner': case 'creator':{
  ...
  let pp = await hardi.profilePictureUrl(`${p}@s.whatsapp.net`, 'image').catch((_) => "https://telegra.ph/file/1ecdb5a0aee62ef17d7fc.jpg");
  let owner = `wa.me/${p}`;
  let vcard = `
BEGIN:VCARD
VERSION:3.0
N:WhatsApp; KaaKangSatir
...
item3.URL:https://store.hardi.biz
...
item5.X-ABADR:💬 More
END:VCARD`;
  const sentMsg = await hardi.sendMessage(m.chat, {
    contacts: {
...
hardi.sendMessage(from, { text : `Hai Kak @${sender.split("@")[0]}, Nih Owner Ku.. Kalo Ada Sesuatu Bilang Aja Sama Dia :3`, contextInfo:{
...
await hardi.relayMessage(m.chat,  {
requestPaymentMessage: {
currencyCodeIso4217: 'IDR',
...
konstak = await hardi.sendMessage(m.chat, {
    text: `Group : *${groupMetadata.subject}*\nMember : *${participants.length}*`
}, {quoted: m, ephemeralExpiration: 86400})
hardi.sendContact(m.chat, participants.map(a => a.id), konstak)
}
break
case 'totag': {
if (!m.isGroup) return reply(mess.group)
...
hardi.sendMessage(m.chat, { forward: m.quoted.fakeObj, mentions: participants.map(a => a.id) })
}
break
...
await loading()
let response = await hardi.groupInviteCode(from)
hardi.sendText(from, `https://chat.whatsapp.com/${response}\n\nLink Group : ${groupMetadata.subject}`, m, { detectLink: true })
}
break
...
let response = await hardi.groupInviteCode(from)
hardi.sendText(bnnd, `https://chat.whatsapp.com/${response}\n\nLink Group : ${groupMetadata.subject}`, m, { detectLink: true })
...
await hardi.groupParticipantsUpdate(from, [users], 'remove')
reply(mess.done)
}
break
case 'add': {
...
await hardi.groupParticipantsUpdate(from, [users], 'add')
reply(mess.done)
}
break
case 'promote': {
...
if (!isAdmins) return reply(mess.admin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await hardi.groupParticipantsUpdate(from, [users], 'promote')
reply(mess.done)
}
break
case 'demote': {
...
if (!isAdmins) return reply(mess.admin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await hardi.groupParticipantsUpdate(from, [users], 'demote')
reply(mess.done)
}
break
case 'hidetag': 
...
if (!m.isGroup) return reply(mess.group)
if (!isAdmins) return reply(mess.admin)
hardi.sendMessage(from, { text : q ? q : '' , mentions: participants.map(a => a.id)}, {quoted: Shun})
}
break
break
case 'tagall': {
...
hardi.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted:fcall })
}
break
...
let list = Object.entries(global.db.data.users)
	...
	list.map(([user, data], i) => (Number(data.limit = lim)))
		hardi.sendMessage(m.chat, {text: `*Limit berhasil direset ${lim} / user*`}, { quoted: m })
		}
break
...
if (!text) return reply('Masukkan Jumlah Limit Yang Akan Diberi')
    ...
    hardi.sendMessage(m.chat, {text: `Selamat @${who.split`@`[0]}. Kamu Mendapatkan +${poin} Limit!`}, {quoted:m})
    }
break
...
let metaDATA = await hardi.groupMetadata(idgc).catch((e) => reply(e))
...
setTimeout(function() {
hardi.sendMessage(getDATA[i], { text: pesan });
count--;
sentCount++;
if (count === 0) {
...
for (let geek of mem) {
await sleep(3000)
hardi.sendMessage(geek, {text: `${teksnye}`}, {quoted:fcall})
}
...
await hardi.groupAcceptInvite(result).then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break

case "cekidgc": {
if (!isCreator) return reply(mess.premium)
let getGroups = await hardi.groupFetchAllParticipating()
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let anu = groups.map((v) => v.id)
let teks = `⬣ *LIST GROUP DI BAWAH*\n\nTotal Group : ${anu.length} Group\n\n`
for (let x of anu) {
let metadata2 = await hardi.groupMetadata(x)
...
case 'backup':
     ...
     await reply("Sabar Mas Lagi Backup!!!");
          ...
          const exec = await execSync(`zip -r Hardi.zip ${ls.join(" ")}`);
          await hardi.sendMessage(
            m.chat,
            {
              document: await fs.readFileSync("./ShunAi.zip"),
              mimetype: "application/zip",
              fileName: "ShunAi.zip",
            },
            { quoted: fcall }
          );
          await execSync("rm -rf Hardi.zip");
        }
        break
...
var cekap = await hardi.onWhatsApp(pcknm+"@s.whatsapp.net")
...
fs.writeFileSync('./ShunAi/database/premium.json', JSON.stringify(premium))
reply('Sukses!')
} else {
var cekpr = await hardi.onWhatsApp(args[0]+"@s.whatsapp.net")
...
hardi.sendTextWithMentions(m.chat, txt, m)
}
break
...
for (let xnxx of anu) {
if (/image/.test(mime)) {
media = await hardi.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(media)
await hardi.sendMessage(xnxx, { image: { url: mem }, caption: text.split('|')[0] })
await sleep(text.split('|')[1])
} else {
await hardi.sendMessage(xnxx, { text: text.split('|')[0] })
await sleep(text.split('|')[1])
}}
...
for (let i of anu) {
await sleep(1500)
hardi.sendMessage(i, {text: `${text}`}, {quoted:fcall})
    }
reply(`Sukses Mengirim Broadcast Ke ${anu.length} Group`)
}
break
...
hardi.sendMessage(m.chat, { text: d.result.lyrics }, { quoted: fcall })
}
break
case 'pinterest': case 'pin': {
  ...
  await hardi.relayMessage(m.chat, msg.message, {
    messageId: msg.key.id
  });
  ...
  viewOnceMessage: {
    message: {
        ...
        interactiveMessage: proto.Message.InteractiveMessage.create({
          ...
          header: proto.Message.InteractiveMessage.Header.create({
                ...(await prepareWAMessageMedia({ image : { url : search.all[0].thumbnail }}, { upload: hardi.waUploadToServer})), 
                  title: ``,
                  gifPlayback: true,
                  subtitle: ownername,
                  hasMediaAttachment: false  
...
await sleep(200)
                ...
                hardi.sendMessage(m.chat, { document: { url: url }, fileName: filename, mimetype: 'application/zip' }, { quoted: m }).catch((err) => reply(mess.error))
            .catch(console.error)
break
...
let media = await hardi.downloadMediaMessage(quoted)
let { toAudio } = require('./lib/converter')
let audio = await toAudio(media, 'mp4')
hardi.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By ${hardi.user.name}.mp3`}, { quoted : m })
//hardi.sendMessage(m.chat, {audio , mimetype: 'audio/mpeg', fileName: `Convert By ${hardi.user.name}.mp3`}, { quoted : m })
}
break
...
let { toAudio } = require('./lib/converter')
let audio = await toAudio(media, 'mp4')
hardi.sendMessage(m.chat, {audio: audio, mimetype: 'audio/mpeg'}, { quoted : m })
}
break
...
hardi.sendMessage(from, {audio: audio, mimetype:'audio/mpeg', ptt:true})
}
break
...
hardi.sendMessage(from ,{ image: krt.result, caption: mess.success },{ quoted: m })
}
break
...
case 'hd': {
			...
			hardi.sendMessage(m.chat, { image: proses, caption: mess.success}, { quoted: fcall})
			}
			break
...
if (isBan) return reply(mess.bann)
    ...
    if (text && m.quoted) {
        if (m.quoted.sender) {
            reply = {
                "name": await hardi.getName(m.quoted.sender),
                "text": m.quoted.text || '',
    ...
    const pp = await hardi.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/320b066dc81928b782c7b.png');
    ...
    if (mime) {
        const obj = {
            ...
            "messages": [{
                ...
                from: {
                    id: 1,
                    name: await hardi.getName(m.sender),
                    photo: {
                        url: pp
                    }
                },
        ...
        if (stickerResult) return hardi.sendImageAsSticker(m.chat, stickerResult, m, { packname: global.packname, 
...
if (/image/.test(mime)) {
let media = await quoted.download()
let encmedia = await hardi.sendImageAsSticker(from, media, m, { packname: pcknm, author: atnm })
...
case 'smeme': case 'stickermeme': case 'stickmeme': {
    ...
    if (!/webp/.test(mime) && /image/.test(mime)) {
        ...
        let mee = await hardi.downloadAndSaveMediaMessage(quoted)
        ...
        let memek = await hardi.sendImageAsSticker(m.chat, meme, m, { packname: global.packname, author: global.author })

    } else {
        reply(`Send/reply image with caption ${prefix + command} Shun|Hardi`)
    }
}
    break
...
await loading()
let media = await quoted.download()
let encmedia = await hardi.sendImageAsStickerAV(from, media, fcall, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return reply('Maksimal 10 detik!')
let media = await quoted.download()
let encmedia = await hardi.sendVideoAsSticker(from, media, fcall, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else {
...
hardi.copyNForward(from, msgs[budy.toLowerCase()], true)}}
} catch (err) {
